    ...
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        const maxTranslationX =
          scale == 1 ? screenWidth * scale - screenWidth / 2 : screenWidth;
        const maxTranslationY = screenWidth * scale - screenWidth / 2;

        // Limit movement to 50% of the screen
        let moveX =
          Math.abs(dx) > maxTranslationX / 3 ? maxTranslationX / 3 : dx;
        let moveY =
          Math.abs(dy) > maxTranslationY / 3 ? maxTranslationY / 3 : dy;
